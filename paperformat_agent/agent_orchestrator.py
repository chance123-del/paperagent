...
The orchestrator never generates manuscript content. It records deterministic
tool work and decides whether a run may proceed to formal delivery.
"""
...
class PaperDeliveryAgent:
    """Persist a run-level plan, decisions and evidence in one JSON trace."""
    ...
    def __init__(self, run_dir: Path, source_name: str, target_name: str) -> None:
        ...
        self.trace: dict[str, Any] = {
            "schema_version": "1.1",
            "agent": "ScholarFlow Delivery Orchestrator",
            "goal": "Produce a traceable manuscript delivery package without inventing research content.",
            "source": source_name,
            "target": target_name or "unspecified",
            "started_at": self._now(),
            "status": "running",
            "steps": [
                asdict(PlanStep("intake", "prepare_project", "Parse the source in a protected workspace.")),
                asdict(PlanStep("rules", "rule_resolver", "Resolve only evidenced formatting and citation rules.")),
                asdict(PlanStep("repair", "analyze_and_repair", "Apply reversible formatting repairs.")),
                asdict(PlanStep("assets", "asset_matcher", "Match figures, tables and confirmed equations.")),
                asdict(PlanStep("verify", "latex_compiler", "Compile or preserve a fidelity-safe preview.")),
                asdict(PlanStep("delivery", "delivery_gate", "Release delivery only after blocking items are resolved.")),
            ],
            "decisions": [],
        }
        self.persist()
    ...
    def complete_step(self, step_id: str, summary: str, evidence: list[str] | None = None) -> None:
        self._step(step_id).update({"status": "completed", "summary": summary, "evidence": evidence or []})
        self.persist()

    def block_step(self, step_id: str, summary: str, evidence: list[str] | None = None) -> None:
        self._step(step_id).update({"status": "blocked", "summary": summary, "evidence": evidence or []})
        self.persist()
    ...
    def finish(self, blockers: list[str], compile_status: str) -> None:
        if blockers:
            ...
            self.decide("Request human review", "Deterministic checks passed; review the preview before formal export.")
    ...
    def to_markdown(self) -> str:
        ...
        lines = ["### 智能编排审计", "", f"- 当前状态：`{labels.get(self.trace['status'], self.trace['status'])}`"]
        for step in self.trace["steps"]:
