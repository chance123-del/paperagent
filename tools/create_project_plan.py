...
def main():
    ...
    table(d, ["输入", "允许形式", "系统要求与处理"], [
        ...
        ("图表注合集包", "单独 ZIP（内含 Excel/CSV/JSON 清单）", "不参与图表文件匹配；系统提供可下载模板，用户填写后上传；只按资产 ID 取回图注、表注、来源注、替代文本与可选链接，再由目标规则决定其位置。"),
    ...
    table(d, ["步骤", "系统动作", "用户决策/界面提醒"], [
        ...
        ("7. 反馈与修改", "接收对象级反馈，生成有证据的差异补丁并建立新版本。", "用户逐项接受；无证据的内容性请求只进入待办/追问。"),
        ("8. 下载", "打包正式交付或明确标识的预览交付。", "展示版本号、输入哈希、规则快照、反馈处理记录和未关闭风险。"),
    ], [2.1, 7.0, 7.1])
    ...
    para(d, "用户可单独上传 annotations.zip，或直接上传系统提供的 annotations.xlsx 模板填写结果。该包是“文字和链接元数据”，不是图片/表格资源包，也不在正文中产生新的图表占位符。正文只保留 [Fig1]、[Table1] 等对象占位符；系统先匹配对象，再以 Fig1/Table1 作为键读取注释记录并附着到对应对象。", indent=True)
    ...
    bullet(d, "平台提供“下载模板”入口：用户可下载与当前项目或当前期刊规则匹配的 annotations.xlsx 空模板，按模板填写后回传；无需自行建表。")
    bullet(d, "系统校验模板版本、sheet 名、字段名、asset_id 和 prefix_policy 枚举值；不合法模板直接报字段级错误，不做静默容错。")
    bullet(d, "当用户已选择期刊模板时，下载模板可带出当前规则提示，例如图题下置、表题上置、是否允许链接、是否要求 alt_text。")
    ...
    heading(d, "5.3.1 数字型原文占位符与动态参考文献", 3)
    para(d, "系统兼容原稿常见的 [1]、[1,3]、[2-4]、[1-3,5] 等数字型引用。导入时，数字只作为“原始参考文献表序号”的解析线索：系统先将其映射为确定的内部文献键（例如 Smith2024），而不把 [1] 当作永久文本。若原始列表第 1 项缺失、重复或顺序不明，该引用进入人工确认队列。", indent=True)
    table(d, ["发生的编辑", "内部动作", "最终稿变化"], [
        ("导入原文 [2-4]", "按原始参考文献表的第 2 至第 4 项转换为三个内部文献键。", "数字制期刊渲染为其要求的连续/合并格式；著者-年份制渲染为对应作者年份。"),
        ("删除一处引用占位符", "从引用图中移除该位置与文献键的关系，重新统计被引条目。", "按期刊规则更新文内编号，并从最终文献表移除已无任何引用的条目；原始合集仍保留。"),
        ("新增一处引用", "用户从已有文献库选择条目，或先导入有完整字段的新条目再插入占位符。", "重新生成全部文内标注及参考文献表，避免手改 [n]。"),
        ("切换期刊/引用样式", "引用 AST 保持文献键关系不变，只更换模板的 cite/bibliography 配置。", "可从 [1] 样式动态变为 (Smith, 2024) 等已确认目标样式。"),
    ], [3.3, 7.2, 5.7])
    bullet(d, "界面新增引用按钮：选择文献后插入当前原稿风格的 [n] 占位符，或显示 [[cite:key]] 技术标记；两者都只映射到同一个内部引用对象。用户不需要自行计算新编号。")
    bullet(d, "最终参考文献表由当前有效引用集合和目标期刊规则动态生成。若期刊明确要求列出未引用文献，规则包必须写明；否则不把无引用条目混入最终稿。")
    note(d, "关键提醒：", "引用占位符只解决“把已有文献准确放入正文”的问题，不能证明该文献支持该论断。论断与文献的学术相关性仍由作者负责。")

    heading(d, "六、反馈与修改页面")
    para(d, "在生成预览和交付包页面增加“反馈与修改”页。用户可点击正文段落、图表、表格、图表注、文中引用、参考文献或格式检查项，或直接输入自然语言疑问。系统用 source mapping 将反馈关联到具体输出对象和其输入来源。", indent=True)
    table(d, ["反馈字段", "用户操作", "系统与大模型职责"], [
        ("定位", "点击对象，或填写页码/图表 ID/引用 ID；可附截图。", "定位到输出节点、规则和原始来源，不能仅靠页码猜测。"),
        ("问题描述", "例如“图 2 太宽”“[3] 应引用 Wang2023”“这里的题注有误”。", "分类为规则、资产、引用、用户提供文本或未知；给出可审查修改计划。"),
        ("证据/替换内容", "用户粘贴正确文字、上传替换文件或选择已有文献。", "仅使用该证据生成补丁；缺少证据时提问或标记待补。"),
        ("修改预览", "查看原文、建议后内容、影响对象和规则依据。", "生成差异，不直接覆盖正式版本；用户可逐项接受、拒绝或继续追问。"),
        ("状态", "草稿、待确认、已接受、已应用、已拒绝。", "保留操作者、时间、版本和构建结果，支持回退。"),
    ], [3.0, 5.6, 7.6])
    bullet(d, "大模型可以把自然语言反馈匹配到对象、规则和用户已提供的替换信息；对纯格式问题可提出规则性补丁。它不得自行补写研究结论、图表题注、参考文献、DOI、链接或事实性段落。")
    bullet(d, "每次接受反馈后生成新项目版本并重新执行引用重编号、参考文献表生成、图表版式检查和 PDF 编译。反馈未解决或新增阻断项时，正式交付状态自动失效。")
    note(d, "反馈页红线：", "“请帮我把这里改好”不是授权模型编造内容。没有明确替换文本、已存在来源或可验证规则的内容修改，系统只能追问、建议或创建待办。", RED)

    heading(d, "七、反编造与质量门禁（项目红线）")
    ...
    table(d, ["阶段", "范围", "完成标志"], [
        ...
        ("P2：图表、引用与确认工作台（3-4 周）", "匹配预览、重复选择、annotations.zip、模板下载/上传校验、图表版式预检、题注/图注/表注/交叉引用及 [[cite:key]] 引用占位符。", "所有图表、图表注、链接和文中引用均可人工确认并生成审计清单。"),
        ("P3：规则、动态引用与渲染（3-4 周）", "规则证据模型、数字引用解析与重编号、官方模板接入、LaTeX 项目构建、PDF/Word/zip 输出。", "新增/删除引用后，文中样式与文献表同步更新；首批模板真实编译。"),
        ("P4：反馈、核验与试运行（2-3 周）", "对象级反馈页、差异预览、回归测试、异常用例、版面检查和用户验收。", "反馈可回退且无证据内容不被自动写入；阻断门禁和报告可复核。"),
    ], [3.4, 8.2, 4.6])
    note(d, "范围控制提醒：", "首版应锁定“文字型初稿 + 规范 ZIP + BibTeX + 少量已核验模板”。OCR、复杂 Word 浮动对象、任意期刊网页自动抓取和自动补全书目应列为后续能力。")

    heading(d, "十一、验收测试集与指标")
    table(d, ["测试类别", "样例", "通过标准"], [
        ("正常路径", "DOCX + [Fig1]/[Table1] + 唯一资产 + 完整 BibTeX + 已验证模板。", "生成四类交付物；资产、编号、引用、规则和 PDF 均通过。"),
        ("动态引用", "原稿含 [1]、[2-4]；删除一处引用并新增一条已导入文献。", "按目标期刊样式重新渲染所有文内引用与参考文献表；无手工遗留编号。"),
        ("反馈闭环", "用户对图表位置、题注、引用和正文分别提交反馈。", "只对有规则或用户证据的项目生成可接受补丁；每次应用后重新核验并可回退。"),
    ...
    table(d, ["待确认事项", "建议默认值", "原因"], [
        ...
        ("题注与图注输入形式", "独立 annotations.zip（Excel/CSV/JSON）+ 系统可下载模板 + 可视化确认表单。", "与 [FigN]/[TableN] 对象占位符分离，避免从图片或临近文字推断。"),
    ...
    para(d, "建议立即进入 P0：确定首批目标模板，收集 5-10 份脱敏样稿及对应资料包，冻结 annotations.zip、manifest.csv 与反馈处理规则，并将本项目书中的发布门禁转为产品验收用例。", indent=True)
