...
def main():
    ...
    table(d, ["成功标准", "可被验收的结果"], [
        ...
        ("资产完整", "每个 [FigN]/[TableN] 的匹配状态明确为已匹配、缺失、重复、类型不符或待人工确认。"),
    ...
    table(d, ["输入", "允许形式", "系统要求与处理"], [
        ...
        ("图表资料包", "ZIP（含图片、表格、可选清单）", "安全解压；拒绝路径穿越、可执行文件、超限文件；按文件名与占位符做精确匹配。"),
    ...
    table(d, ["文件", "用途", "发布条件"], [
        ...
        ("asset_manifest.csv/json", "图表/表格溯源", "列出占位符、原文件、哈希、题注来源、链接来源、输出标签和状态。"),
    ], [3.0, 5.2, 8.0])

    heading(d, "三、端到端工作流与交互")
    table(d, ["步骤", "系统动作", "用户决策/界面提醒"], [
        ("1. 新建项目", "建立隔离工作目录，计算上传文件哈希，生成 project.json。", "提示项目名称、目标期刊/学校、匿名稿要求及输出语言。"),
        ("2. 上传与解析", "抽取正文、标题层级、已有图表、占位符、引用与文献；扫描 ZIP。", "显示解析覆盖率；提示扫描件 OCR、加密文件和未识别格式。"),
        ...
        ("5. 编排", "只做格式变换、资产插入、编号与引用渲染；生成中间 LaTeX。", "所有将改变正文文本的动作默认关闭；用户可逐项确认规则性修复。"),
    ...
    table(d, ["推荐文件", "作用", "是否必需"], [
        ...
        ("captions.yaml", "显式给出 caption、note、source、alt_text。", "强烈推荐；期刊要求题注时实际必需"),
        ("links.yaml", "显式给出相关 URL/DOI、显示文字、链接用途。", "仅有外链需求时必需"),
        ("manifest.csv", "资产 ID、文件、类型、版本、校验和、确认状态。", "批量/多人协作推荐"),
    ], [3.5, 9.1, 3.6])
    heading(d, "4.2 匹配算法与状态机", 2)
    table(d, ["规则", "行为", "结果"], [
    ...
    heading(d, "4.3 题注、图注、表注与交叉引用", 2)
    bullet(d, "题注（caption）来源优先级固定为：用户在 captions.yaml/表单明确填写 > 初稿中与占位符绑定的已存在题注 > 空值并报待补。系统不得根据图片内容自动写题注。")
    ...
    heading(d, "4.4 超链接策略", 2)
    table(d, ["链接类型", "安全实现", "禁止行为"], [
        ("DOI/文献 URL", "只从 BibTeX、links.yaml 或作者表单读取；使用 hyperref 生成可点击链接。", "根据题名搜索并猜填 DOI/URL。"),
        ("图表来源/数据仓库", "通过 links.yaml 绑定资产 ID，显示用户提供的来源文字或 DOI。", "将图片像素识别结果、文件名或网页猜测为链接。"),
    ...
    table(d, ["现有基础", "V1 需补齐", "验收重点"], [
        ...
        ("图/表 LaTeX 插入", "加入 captions.yaml、notes、alt_text、稳定 label、交叉引用、links.yaml 与模板化超链接。", "题注和链接均能追溯来源。"),
    ...
    table(d, ["阶段", "范围", "完成标志"], [
        ...
        ("P2：图表与确认工作台（3-4 周）", "匹配预览、重复选择、captions.yaml/links.yaml、题注/图注/交叉引用。", "所有图表和链接均可人工确认并生成审计清单。"),
    ...
    table(d, ["待确认事项", "建议默认值", "原因"], [
        ...
        ("题注与图注输入形式", "ZIP 内 captions.yaml + 可视化确认表单。", "避免从图片或临近文字推断。"),
        ("链接政策", "只接受用户提供的 DOI/URL；仅做格式和可访问性检查。", "链接有学术与隐私风险，不能自动猜填。"),
